...
Logger.prototype._applySerializers = function (fields, excludeFields) {
    ...
    Object.keys(this.serializers).forEach(function (name) {
        var fieldStart = process.hrtime();
        
        var checkExcludeStart = process.hrtime();
        if (fields[name] === undefined ||
            (excludeFields && excludeFields[name]))
        {
            console.log('Field "%s" is undefined or excluded', name);
            return;
        }
        var checkExcludeTime = process.hrtime(checkExcludeStart);
        console.log('Check exclude field "%s" took %d nanoseconds', 
            name, checkExcludeTime[0] * 1e9 + checkExcludeTime[1]);

        xxx('_applySerializers; apply to "%s" key', name)
        try {
            var totalStart = process.hrtime();
            var serializeStart = process.hrtime();
            var serializer = self.serializers[name];  // Get serializer function
            // fields[name] = self.serializers[name](fields[name]); 
            var serializeTime = process.hrtime(serializeStart);
            console.log('Lookup serializer for field "%s" took %d nanoseconds', 
                name, serializeTime[0] * 1e9 + serializeTime[1]);

            var fieldStart = process.hrtime();
            var fieldValue = fields[name];  // Get field value
            var fieldTime = process.hrtime(fieldStart);
            console.log('Field value:', JSON.stringify(fieldValue));
            console.log('Get field value took:', fieldTime[0] * 1e9 + fieldTime[1]);

            var actualStart = process.hrtime();
            var result = serializer(fieldValue);  
            var actualTime = process.hrtime(actualStart);
            console.log("result:", result);
            console.log('Actual serialization took:', actualTime[0] * 1e9 + actualTime[1]);

            var assignStart = process.hrtime();
            fields[name] = result;  // Assign result
            var assignTime = process.hrtime(assignStart);
            console.log('Assign result took:', assignTime[0] * 1e9 + assignTime[1]);

            var totalTime = process.hrtime(totalStart);
            console.log('Total time processing field "%s" took %d nanoseconds total', 
                name, totalTime[0] * 1e9 + totalTime[1]);
        } catch (err) {
        ...
        var fieldTime = process.hrtime(fieldStart);
        console.log('Processing field "%s" took %d nanoseconds total', 
            name, fieldTime[0] * 1e9 + fieldTime[1]);
    });

    var totalTime = process.hrtime(start);
    console.log('_applySerializers took %d nanoseconds total', 
        totalTime[0] * 1e9 + totalTime[1]);
}
...
function mkRecord(log, minLevel, args) {
    ...
    if (recFields) {
        ...
        if (log.serializers) {
            Object.keys(log.serializers).forEach(function(key) {
                console.log(key + ' serializer:', log.serializers[key].toString());
            });
...
var errSerializer = Logger.stdSerializers.err = function (err) {
    ...
    console.log('Serializing message took %d nanoseconds', messageTime[0] * 1e9 + messageTime[1]);
    
    var nameStart = process.hrtime(); 
    obj.name = err.name;
    var nameTime = process.hrtime(nameStart);
    console.log('Serializing name took %d nanoseconds', nameTime[0] * 1e9 + nameTime[1]);
    
    var stackStart = process.hrtime();
    obj.stack = getFullErrorStack(err);
    var stackTime = process.hrtime(stackStart);
    console.log('Serializing stack took %d nanoseconds', stackTime[0] * 1e9 + stackTime[1]);
    
    var codeStart = process.hrtime();
    obj.code = err.code;
    var codeTime = process.hrtime(codeStart);
    console.log('Serializing code took %d nanoseconds', codeTime[0] * 1e9 + codeTime[1]);
    
    var signalStart = process.hrtime();
    obj.signal = err.signal;
    var signalTime = process.hrtime(signalStart);
    console.log('Serializing signal took %d nanoseconds', signalTime[0] * 1e9 + signalTime[1]);

    var totalTime = process.hrtime(start);
    console.log('Total serialization took %d nanoseconds', totalTime[0] * 1e9 + totalTime[1]);
