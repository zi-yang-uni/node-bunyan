...
var xxx = function xxx(s) {     // internal dev/debug logging
    ...
    console.log('xxx took %d nanoseconds', end[0] * 1e9 + end[1]);
};
...
if (runtimeEnv === 'browser') {
    os = {
        hostname: function () {
            ...
            var end = process.hrtime(start);
            console.log('hostname took %d nanoseconds', end[0] * 1e9 + end[1]);
            return result;
        }
    };
    fs = {};
...
function objCopy(obj) {
    ...
    if (obj == null) {  // null or undefined
        var end = process.hrtime(start);
        console.log('objCopy took %d nanoseconds', end[0] * 1e9 + end[1]);
        return obj;
...
if (!format) {
    ...
    format = function format(f) {
        ...
        if (typeof (f) !== 'string') {
            ...
            var result = objects.join(' ');
            var end = process.hrtime(start);
            console.log('format took %d nanoseconds', end[0] * 1e9 + end[1]);
            return result;
        }
...
function getCaller3Info() {
    ...
    if (this === undefined) {
        ...
        var end = process.hrtime(start);
        console.log('getCaller3Info took %d nanoseconds', end[0] * 1e9 + end[1]);
        return;
    }
    var obj = {};
...
function _indent(s, indent) {
    ...
    if (!indent) indent = '    ';
    var lines = s.split(/\r?\n/g);
    var result = indent + lines.join('\n' + indent);
    var end = process.hrtime(start);
    console.log('_indent took %d nanoseconds', end[0] * 1e9 + end[1]);
    return result;
}
...
function _warn(msg, dedupKey) {
    ...
    if (dedupKey) {
        if (_warned[dedupKey]) {
            var end = process.hrtime(start);
            console.log('_warn took %d nanoseconds', end[0] * 1e9 + end[1]);
            return;
        }
...
function _haveWarned(dedupKey) {
    var start = process.hrtime();
    var result = _warned[dedupKey];
    var end = process.hrtime(start);
    console.log('_haveWarned took %d nanoseconds', end[0] * 1e9 + end[1]);
    return result;
}
var _warned = {};
...
ConsoleRawStream.prototype.write = function (rec) {
    ...
    var end = process.hrtime(start);
    console.log('ConsoleRawStream.write took %d nanoseconds', end[0] * 1e9 + end[1]);
};
...
function resolveLevel(nameOrNum) {
    ...
    var end = process.hrtime(start);
    console.log('resolveLevel took %d nanoseconds', end[0] * 1e9 + end[1]);
    return level;
}
...
function isWritable(obj) {
    ...
    if (obj instanceof stream.Writable) {
        ...
        result = typeof (obj.write) === 'function';
    }
    var end = process.hrtime(start);
    console.log('isWritable took %d nanoseconds', end[0] * 1e9 + end[1]);
    return result;
}
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (!(this instanceof Logger)) {
        var end = process.hrtime(start);
        console.log('Logger constructor took %d nanoseconds', end[0] * 1e9 + end[1]);
...
Logger.prototype.addStream = function addStream(s, defaultLevel) {
    ...
    var end = process.hrtime(start);
    console.log('addStream took %d nanoseconds', end[0] * 1e9 + end[1]);
}
...
Logger.prototype.addSerializers = function addSerializers(serializers) {
    ...
    var end = process.hrtime(start);
    console.log('addSerializers took %d nanoseconds', end[0] * 1e9 + end[1]);
}
...
Logger.prototype.child = function (options, simple) {
    var start = process.hrtime();
    var result = new (this.constructor)(this, options || {}, simple);
    var end = process.hrtime(start);
    console.log('child took %d nanoseconds', end[0] * 1e9 + end[1]);
    return result;
}
...
Logger.prototype.reopenFileStreams = function () {
    ...
    var end = process.hrtime(start);
    console.log('reopenFileStreams took %d nanoseconds', end[0] * 1e9 + end[1]);
};
...
Logger.prototype.level = function level(value) {
    ...
    if (value === undefined) {
        var end = process.hrtime(start);
        console.log('level get took %d nanoseconds', end[0] * 1e9 + end[1]);
        return this._level;
    }
    ...
    var end = process.hrtime(start);
    console.log('level set took %d nanoseconds', end[0] * 1e9 + end[1]);
}
...
function mkRecord(log, minLevel, args) {
    ...
    if (args[0] instanceof Error) {
        ...
        var errEnd = process.hrtime(errStart);
        console.log('err took %d nanoseconds', errEnd[0] * 1e9 + errEnd[1]);
    ...
    var recStart = process.hrtime();
    var copyStart = process.hrtime();
    var rec = objCopy(log.fields);
    var copyEnd = process.hrtime(copyStart);
    console.log('objCopy took %d nanoseconds', copyEnd[0] * 1e9 + copyEnd[1]);
    var level = rec.level = minLevel;
    var recFieldsStart = process.hrtime();
    var recFields = (fields ? objCopy(fields) : null);
    var recFieldsEnd = process.hrtime(recFieldsStart);
    console.log('recFieldsCopy took %d nanoseconds', recFieldsEnd[0] * 1e9 + recFieldsEnd[1]);
    if (recFields) {
        var serializeStart = process.hrtime();
        var serializersStart = process.hrtime();
        if (log.serializers) {
            log._applySerializers(recFields, excludeFields);
        }
        var serializeEnd = process.hrtime(serializeStart);
        console.log('serialize took %d nanoseconds', serializeEnd[0] * 1e9 + serializeEnd[1]);
        var applyStart = process.hrtime();
        Object.keys(recFields).forEach(function (k) {
            rec[k] = recFields[k];
        });
        var applyEnd = process.hrtime(applyStart);
        console.log('apply took %d nanoseconds', applyEnd[0] * 1e9 + applyEnd[1]);
    }
    var recFieldsEnd = process.hrtime(recFieldsStart);
    console.log('recFields took %d nanoseconds', recFieldsEnd[0] * 1e9 + recFieldsEnd[1]);
    var msgStart = process.hrtime();
    rec.msg = format.apply(log, msgArgs);
    var msgEnd = process.hrtime(msgStart);
    console.log('format took %d nanoseconds', msgEnd[0] * 1e9 + msgEnd[1]);
    if (!rec.time) {
        var timeStart = process.hrtime();
        rec.time = (new Date());
        var timeEnd = process.hrtime(timeStart);
        console.log('new Date took %d nanoseconds', timeEnd[0] * 1e9 + timeEnd[1]);
    }
    // Get call source info
    if (log.src && !rec.src) {
        var srcStart = process.hrtime();
        rec.src = getCaller3Info()
        var srcEnd = process.hrtime(srcStart);
        console.log('getCaller3Info took %d nanoseconds', srcEnd[0] * 1e9 + srcEnd[1]);
    }
    rec.v = LOG_VERSION;
    var recEnd = process.hrtime(recStart);
    console.log('rec took %d nanoseconds', recEnd[0] * 1e9 + recEnd[1]);

    var end = process.hrtime(start);
    console.log('mkRecord took %d nanoseconds', end[0] * 1e9 + end[1]);
    return rec;
};
...
function mkLogEmitter(minLevel) {
    return function () {
        console.log('BUNYAN-TEST: mkLogEmitter called with level:', minLevel);
        var start = process.hrtime();
        var log = this;
        var str = null;
        var rec = null;
        ...
        var end = process.hrtime(start);
        console.log('mkLogEmitter took %d nanoseconds', end[0] * 1e9 + end[1]);
    }
}
...
var errSerializer = Logger.stdSerializers.err = function (err) {
    ...
    var messageStart = process.hrtime();
    obj.message = err.message;
    var messageTime = process.hrtime(messageStart);
    
    var nameStart = process.hrtime(); 
    obj.name = err.name;
    var nameTime = process.hrtime(nameStart);
    
    var stackStart = process.hrtime();
    obj.stack = getFullErrorStack(err);
    var stackTime = process.hrtime(stackStart);
    
    var codeStart = process.hrtime();
    obj.code = err.code;
    var codeTime = process.hrtime(codeStart);
    
    var signalStart = process.hrtime();
    obj.signal = err.signal;
    var signalTime = process.hrtime(signalStart);

    var totalTime = process.hrtime(start);
    
    obj._timings = {
        message: messageTime[0] * 1e9 + messageTime[1],
        name: nameTime[0] * 1e9 + nameTime[1], 
        stack: stackTime[0] * 1e9 + stackTime[1],
        code: codeTime[0] * 1e9 + codeTime[1],
        total: totalTime[0] * 1e9 + totalTime[1]
    };
...
function safeCyclesSet() {
    ...
    var replacer = function (key, val) {
        ...
        var fnEnd = process.hrtime(fnStart);
        console.log('safeCyclesSet replacer took %d nanoseconds', fnEnd[0] * 1e9 + fnEnd[1]);
        return result;
    };
    var end = process.hrtime(start);
    console.log('safeCyclesSet setup took %d nanoseconds', end[0] * 1e9 + end[1]);
    return replacer;
}
...
function safeCyclesArray() {
    ...
    var replacer = function (key, val) {
        ...
        var fnEnd = process.hrtime(fnStart);
        console.log('safeCyclesArray replacer took %d nanoseconds', fnEnd[0] * 1e9 + fnEnd[1]);
        return result;
    };
    var end = process.hrtime(start);
    console.log('safeCyclesArray setup took %d nanoseconds', end[0] * 1e9 + end[1]);
    return replacer;
}
